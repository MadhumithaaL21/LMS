...
function Todo() {
  ...
  useEffect(() => {
    async function fetchTodos() {
      const response = await axios.get("https://mernback-7nkl.onrender.com/", {
  ...
  const handleNewTodoSubmit = async (event) => {
    ...
    const response = await axios.post("https://mernback-7nkl.onrender.com/", {
      text: newTodo,
      user_id: user._id,
    });
  ...
  const handleTodoDelete = async (id) => {
    await axios.delete(`https://mernback-7nkl.onrender.com/${id}`);
